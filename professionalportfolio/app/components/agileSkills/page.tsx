...
export default function AgileSkillsComponent({ searchQuery }: { searchQuery: string }) {
    ...
    useEffect(() => {
        if (agileSkills) {
            const filteredAgileSkills = agileSkills.agile.filter(skill => skill.toLowerCase().includes(searchQuery));
            const filteredCertifications = agileSkills.certification.filter(cert => cert.toLowerCase().includes(searchQuery));

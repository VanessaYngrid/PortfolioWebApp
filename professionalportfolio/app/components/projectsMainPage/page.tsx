            ...
            const component = (
                <div className="flex flex-col lg:flex-row w-full px-8 lg:px-14">
                    ...
                    <div className="w-full lg:w-1/2 flex flex-col justify-center text-start py-4 px-0 md:py-6 lg:px-20">
                        <h2 className="text-gray-800 text-2xl font-semibold pb-4">{title}</h2>
                        <p className="text-gray-700">{category}</p>
                        <p className="text-gray-700 pb-6"><strong>Duration:</strong> {duration}</p>
                        <div className="flex flex-wrap">
                            {technologies.map((tech) => (
                                <span key={tech} className="inline-block bg-[#8A3D57] bg-opacity-75 rounded-md py-1 text-sm font-normal px-2 text-[#F9F9F9] mr-2 mb-2">
                                    #{tech}
                                </span>
                            ))}
                        </div>
